...
The image has a resolution of 30x30 meters and has been acquired by Landsat 8 the 2021/06/14. It is provided with `geocmeans` as a Large RasterBrick `Arcachon`.
...
we can start with a simple visualisation of the dataset
...
It seems that `beta = 0.5` gives the best results wiht `m = 1.5`, we gain 2 more percentage points of inertia and keeping the same Silhouette index.
...
We can see that with the same threshold, we obtain far less undecided pixels and only little variations in comparison with the previous results.
...
We have to determine the right value of alpha and the size of the window.
...
To map the results of the SGFCM, it is necessary to lower the threshold of the parameter `undecided`. In comparison with the previous maps, we obtain a much more smooth result which can be easier to use for mapping purpose or delimiting areas. As the drop in the explained inertia suggest, We lost a lot of details. This is not surprising if we consider that the pixels have a size of 30 meters and if we apply a window, the neighbours can be very different from the center of the window. Moreover, the group 4 is not describing anymore the wet sand, but interstitial areas between low and high density urban areas.
...
```
Here, the groups 3 and 7 have the lowest levels of spatial autocorrelation, meaning that these groups tend to be more surrounded by pixels from other groups. Both groups are urban area (high and low density) and spatially close, that's why they have lower Moran I values. We could explore the local Moran I values for both of them with the function `calc_local_moran_raster`. This function gives the same results as `raster::MoranLocal` but is significantly faster.
...
# mapping the values
tm_shape(loc_moran3) + 
  tm_raster(palette = pal, style = "kmeans", n = 9)
...
tm_shape(diagGFCM$Elsa) + 
  tm_raster(palette = "Greys", style = "kmeans", n = 7)
```
...
tm_shape(fuzzy_elsa_rast) + 
  tm_raster(palette = "Greys", style = "kmeans", n = 7)
```
In this case, we can see that the fuzzy ELSA draws more important limits along the coasts, and that the differences between deep and not deep water are weaker. We also have strong artefacts at the South-Est (black dots), which are in reality long buildings in a industrial and commercial area.
