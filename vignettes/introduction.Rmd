...
By ploting the R2 of the kmeans classification for each k between 2 and 10, we can see a fort elbow at k=3. But this small number of group leads to a classification explaining only 43% of the original data variance. With k=5, we reach 50% of the variance, so we decide here to keep k=5.

Let's map the obtained groups.

```{r warning=FALSE}
KMeanClust <-  kmeans(Data,centers=5,iter.max = 150)
LyonIris$KMeanClust <- KMeanClust$cluster
Colors <- recode(LyonIris$KMeanClust,
                 "1"="palegreen3",
                 "2"="firebrick",
                 "3"="lightyellow2",
                 "4"="steelblue",
...
#mapping the groups
sp::plot(LyonIris,col=Colors ,main=paste("Kmean clustering, 5 groups"),cex.main=0.8)
legend("topleft", legend=paste("cluster",1:5), fill=c("palegreen3","firebrick","lightyellow2","steelblue","pink"), cex=0.8,bty="n")
```
...
The classical cmeans will be a comparison point for the spatial cmeans latter. The package geocmeans proposes a function to apply this algorithm, but we will use here the one of the *fcm* package. We fixe the fuzzyness degree (m) to 1.5
...
We can now use the function *calcqualityIndexes* wich combine many indices from the package *fclust* to analyze the quality of the classification

```{r warning=FALSE}
calcqualityIndexes(Data,Cmean$u,Cmean$v)
```

These values will be helpfull in comparison with the ones we will get from SFCM. Modifying the cmeans algorithm the increase spatial consistency leads to a decrease in the quality of the classification.
...
for(alpha in seq(0,2,0.1)){
  print(paste("Calculating alhpa = ",alpha,sep=""))
  #calculating the clustering
  Result <- SFCMeans(Data,WMat,5,m,alpha=alpha,tol=0.00001)
  #calculating the quality indexes
  Indices <- calcqualityIndexes(Result$Data,Result$Belongings,Result$Centers)
  Indices$alpha <- alpha
...
Now we cas check the indices to select the best alpha: achieving a nice spatial consistency increase with a low decrease in classification quality.

Let's start with the spatial consistency. This indicator (developped for this package) proposes to calculate the sum the squared differences between each observation and its neighbours for their belonging probabilities to each cluster. Thus, the maximum for each observation is 4*j with j the number of neighbours for the observation. This maximum is reached if the the observations have 100% chance to belong to a cluster that is different from all its neighbours. So, when we sum up all the values obtained for all the observatiobs, we obtain a quantity of spatial inconsistency. This quantity is divided by the quantity obtained when permutating randomly the rows of the beloning matrix. This second quantity represents the spatial inconsistency that we might expect if the clusters show no spatial consistency. We can repeat the permutation step and keep the mean of the ratios to have a more robus indicators (see help(spConsistency) for details).
...
As expected, the explaiend inertia decrease when alpha increase. The classification has to find a compromise betweem the original values and the lagged values. However, the loss is very small here : only 3% between alpha=0 and alpha=2.
...
After considering all the previous charts, we decide to keep alpha = 1.2 as it seems to give a good balance between spatial consistency and calssification quality in this case.

```{r warning=FALSE}
OkAlpha <- 1.2
Result2 <- SFCMeans(Data,WMat,5,m,alpha=OkAlpha,tol=0.0001, standardize = F)
```

Again, we can map the results of the algorithm. Unsurpisingly, we obtain similar results, but the spatial patterns seems more pronounced.
...
There is still some mixing between the clusters 5 and 1, but the frontiers are way more clear than in the classical cmeans.

To confirm ths observation, we can do a spatial diagnostic with the function *spatialdiag* of the geocmeans package. We will compare the results of the classical and the spatial solutions

```{r warning=FALSE}
spdiag_1 <- spatialDiag(Cmean$u,nblistw = WMat,nrep=30)
spdiag_2 <- spatialDiag(Result2$Belongings,nblistw = WMat,nrep=30)
...
So with the spatial cmeans we achieve a way higher spatial consistency and this is confirmed by the Moran I values calculated on the belonging matrices.

We can map the undiced observations of the final solution. These entities should be analyzed more precisely. Selecting them is easy with the function *undecidedUnits* of the *geocmeans* package.

```{r warning=FALSE}
Undecided <- undecidedUnits(Result2$Belongings,0.4)
Colors <- recode(Undecided,
                 "1"="palegreen3",
                 "2"="firebrick",
                 "3"="lightyellow2",
                 "4"="steelblue",
                 "5"="darkgrey",
                 "Undecided"="black")
sp::plot(LyonIris,col=Colors ,main=paste("Cmean clustering, 5 group"),cex.main=0.8)
legend("topleft", legend=c(paste("cluster",1:5),"undecided"), fill=c("palegreen3","firebrick","lightyellow2","steelblue","darkgrey","black"), cex=0.8,bty="n")
```

Finally, one can obtain many descriptive information about the final groups with two functions of *geocmeans* : 
...
* DescribGroups : calculate summary statistic and display many usefull charts (but use a hard-classification)
...
LyonIris$Groups <- Result2$Groups
describGroups(LyonIris@data, "Groups", AnalysisFields, dec=3)

```

That's all floks ! These are the enhancement for the next version

* support all geometries in the mapping functions
