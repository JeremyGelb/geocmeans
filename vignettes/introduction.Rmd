...
The original paper describing the method can be found here : [https://doi.org/10.1016/j.patcog.2006.07.011](https://doi.org/10.1016/j.patcog.2006.07.011).

The method is discussed and compared to other classification algorithms in the article [Contribution of the spatial c-means fuzzy classification in geography: a socio-residential and environmental taxonomy in Lyon](http://journals.openedition.org/cybergeo/36414) (in frensh).

There are actually numerous packages and functions to perform unsupervised classification in R (*hclust*, *kmeans*, *cmeans*, *factoextra*, etc.). However theses methods are not always well suited to analyze spatial data. Indeed, they do not take into account spatial information like proximity or contiguity between observations. This may lead to solutions for which close observations end up in different groups despite the fact that they are very similar.

The package **ClustGeo** is to our knowledge the only package proposing an unsupervised classification method considering directly spatial proximity between observations. The proposed approach is very appealing because the user has to select a parameter (*alpha*) that controls the weight of the spatial distance matrix (calculated between observations with their locations) VS the semantic distance matrix (calculated  between observations with their variables).
...
The package **geocmeans** is an implementation in R of these methods (originally developed for analysis of brain imagery). It comes with a set of functions to facilitate the analysis of the final belonging matrices : 
...
**geocmeans** comes with a toy dataset *LyonIris*, combining many demographic and environmental variables aggregated at the scale of the Iris in Lyon (France).

Before starting the analysis, it is required to standardize the data because most of the calculus is based on Euclidean distance (we plan to also include Manhattan distance in a future release).
...
```{r warning=FALSE, fig.cap = "Impact of the number of group on the explained variance", out.width = "50%"}
...
By plotting the R-squarred of the kmeans classification for each k between 2 and 10, we can see a first elbow at k=3. But this small number of group leads to a classification explaining only 43% of the original data variance. We decide to keep k=4 to have one more group in the analysis.

Let us map the obtained groups.

```{r warning=FALSE, fig.cap="Clusters identified by classical k-means"}
...
We could know compare this solution with a classical c-means algorithm.

## classical c-means and generalized c-means

The classical c-means is a simple method to perform fuzzy unsupervised classification. The package **geocmeans** proposes the function *CMeans*. We set the fuzzyness degree (m) to 1.5.

We do not present here the algorithm, but only the two main formulas.

The first one is used to update the values of the belonging matrix at each iteration $u_ik$
...
* $m$ the fuzzyness index

```{r warning=FALSE}
Cmean <- CMeans(Data,4,1.5,500,standardize = FALSE, seed = 456, tol = 0.00001, verbose = FALSE)
```

We can now use the function *calcqualityIndexes* which combines many indices from the package **fclust** to analyze the quality of the classification. We will use these values latter to do comparisons among the different algorithms.

```{r message=FALSE, warning=FALSE}
calcqualityIndexes(Data, Cmean$Belongings, m = 1.5)
```

Note, **geocmeans** proposes also a generalized version of the c-means algorithm. It is known to accelerate convergence and yield less fuzzy results by adjusting the belonging matrix at each iteration. It requires an extra parameters *beta* controlling the strength of the modification. The modification affects only the formula updating the belonging matrix.
...
knitr::kable(beta_values[c("beta","Silhouette.index","XieBeni.index","Explained.inertia")],
             col.names = c("beta", "silhouette index", "Xie and Beni index", "explained inertia"),
             digits = 3)
```

Considering the table above, we select beta = 0.7, it keeps a good silhouhette index, and increases Xie and Beni index and explained inertia. Let us compare the results of GFCM and FCM.
...
```
The results indicate that the GFCM gives a solution that is less fuzzy (higher explained inertia and lower partition entropy) but keeps a good silhouette index and an even better Xie and Beni index.

We can now map the two belonging matrices and the most likely group for each observation. To do so, we use the function *mapClusters* from **geocmeans**. We propose here to define a threshold of 0.45. If an observation has only values below this probability in a belonging matrix, it will be labeled as "undecided" (represented with transparency on the map).

We can compare the maps of the classical c-means and the generalized version.
```{r warning=FALSE, fig.cap = "Probability of belonging to cluster 1", out.width="80%"}
...
```
```{r warning=FALSE, fig.cap = "Probability of belonging to cluster 2", out.width="80%"}
ggarrange(cmeansMaps$ProbaMaps[[2]],GcmeansMaps$ProbaMaps[[2]], 
          nrow = 1, ncol = 2, common.legend = TRUE, legend = "bottom")
```
```{r warning=FALSE, fig.cap = "Probability of belonging to cluster 3", out.width="80%"}
...
ggarrange(cmeansMaps$ProbaMaps[[4]],GcmeansMaps$ProbaMaps[[4]], 
          nrow = 1, ncol = 2, common.legend = TRUE, legend = "bottom")
```
```{r warning=FALSE, fig.cap = "Most likely clusters and undecided units", out.width="80%"}
...
The main challenge with the SFCM method is to select the parameter *alpha*. It represents the weight of the spatial dimension (lagged values) in the calculus of the belonging matrix and the cluster centers.
...
The two following formulas show how the functions updating the belonging matrix and the centers of the clusters are modified.
...
As the formula suggests, the *SFCM* can be see as a spatially smoothed version of the classical c-means and *alpha* controls the degree of spatial smoothness. This smoothing can be interpreted as an attempt to reduce spatial overfitting of the classical c-means. 

To select *alpha*, we propose to check all the possible values between 0 and 2 with a step of 0.05.
...
Now we can check the indices to select the best *alpha*. The goal is to reduce as much as possible spatial inconsistency and to keep a good classification quality.

Let's start with the spatial inconsistency. This indicator (developed for this package) calculates the sum the squared differences between each observation and its neighbors on the belonging matrix. Thus, the maximum for each observation is $k*j$ with *j* the number of neighbours for the observation and *k* the number of groups. A maximum is reached if each observations have 100% chance to belong to a cluster that is different from all its neighbours. So, when we sum up the values obtained for all the observations, we obtain a quantity of spatial inconsistency. This quantity is divided by the quantity obtained when permuting randomly the rows of the belonging matrix. This second quantity represents the spatial inconsistency that we might expect if the observations were randomly scattered in space. We can repeat the permutation step (Monte Carlo approach) and keep the mean of the ratios to have a more robust indicators (see help(spConsistency) for details).

A smaller value indicates a smaller spatial inconsistency and thus a greater spatial consistency. 0 meaning that all observations have exactly the same values in the belonging matrix than their neighbours (perfect spatial consistency).

```{r warning=FALSE, fig.cap = "Link between alpha and spatial inconsistency", out.width="50%"}
...
Unsurprisingly, increasing *alpha* leads to a decrease of the spatial inconsistency. This gain follow an inverse function.

Let's check now the explained inertia
```{r warning=FALSE, fig.cap = "Link between alpha and explained inertia", out.width="50%"}
ggplot(DFindices_SFCM)+
  geom_smooth(aes(x=alpha,y=Explained.inertia), color = "black")+
  geom_point(aes(x=alpha,y=Explained.inertia), color = "red")
```

As expected, the explained inertia decreases when alpha increase and follow again an inverse function. The classification has to find a compromise between the original values and the lagged values. However, the loss is very small here: only 3% between alpha = 0 and alpha = 2.

To finish here, we can observe the silhouette and Xie and Beni indicators.

```{r fig.cap="Link between alpha and silhouette index", message=FALSE, warning=FALSE, out.width="50%"}
ggplot(DFindices_SFCM)+
  geom_smooth(aes(x=alpha,y=Silhouette.index), color = "black")+
  geom_point(aes(x=alpha,y=Silhouette.index), color = "red")
```

```{r fig.cap="Link between alpha and Xie and Beni index", message=FALSE, warning=FALSE, out.width="50%"}
ggplot(DFindices_SFCM)+
...
The detail of the meaning of theses indicators is beyond the scope of this vignette. Let's just stress that a bigger silhouette index indicates a better classification, and a smaller Xie and beni index indicates a better classification.

After considering all the previous charts, we decide to keep alpha = 0.7 as it seems to give a good balance between spatial consistency and classification quality in this case.
...
It is also possible to use the generalized version of the spatial c-means. In that case, we have to define both *alpha* and *beta*.

The next formula shows how the belonging matrix is updated at each iteration. Note thate the centers of the clusters are updated with the same formula as SFCM.

$$u_{ik} = \frac{(||x_{k} - v{_i}||^2 -\beta_k + \alpha||\bar{x_{k}} - v{_i}||^2)^{(-1/(m-1))}}{\sum_{j=1}^c(||x_{k} - v{_j}||^2 -\beta_k + \alpha||\bar{x_{k}} - v{_j}||^2)^{(-1/(m-1))}}$$

because we select a small resolution for our grid search of *alpha* and *beta*, we will use a multiprocessing approach.
...
```{r warning=FALSE, fig.cap = "Impact of beta and alpha on silhouette index", out.width = "80%"}
ggplot(DFindices_SFGCM) + 
  geom_raster(aes(x = alpha, y = beta, fill = Silhouette.index),  size = 5) + 
  scale_fill_viridis() +
  coord_fixed(ratio=1)
```
```{r warning=FALSE, fig.cap = "Impact of beta and alpha on Xie and Beni index", out.width = "80%"}
ggplot(DFindices_SFGCM) + 
  geom_raster(aes(x = alpha, y = beta, fill = XieBeni.index),  size = 5) + 
  scale_fill_viridis() +
  coord_fixed(ratio=1)
```
```{r warning=FALSE, fig.cap = "Impact of beta and alpha on spatial inconsistency", out.width = "80%"}
ggplot(DFindices_SFGCM) + 
...
The first two plots indicates that some particular combinaisons of *alpha* and *beta* yield good results in the range 0.8 < *alpha* < 1.2 and 0.6 < *beta* < 0.8. The last plot shows that the selection of *beta* has no impacts on the spatial consistency.

Considering the previous plots, we decide to retain the solution with *beta* = 0.65 and *alpha* = 0.95 which yield very good results for all the considered indices.
...
Again, we compare the here the generalized and the classical version of the spatial c-means algorithm.

```{r warning=FALSE}
r1 <- calcqualityIndexes(Data, SFCM$Belongings,m = 1.5)
r2 <- calcqualityIndexes(Data, SGFCM$Belongings,m = 1.5)

diagSFCM <- spatialDiag(belongmatrix = SFCM$Belongings, nblistw = WMat,undecided = 0.45,nrep = 500)
diagSGFCM <- spatialDiag(belongmatrix = SGFCM$Belongings, nblistw = WMat,undecided = 0.45,nrep = 500)
...
ggarrange(SFCMMaps$ProbaMaps[[4]],SGFCMMaps$ProbaMaps[[4]], nrow = 1, ncol = 2,
          common.legend = TRUE, legend = "bottom")
```
```{r warning=FALSE, fig.cap = "Most likely cluster", out.width="80%"}
...
knitr::kable(moran_table, digits = 3,
             ...
             caption = "Moran I index for the columns of the belonging matrix"
             )
```

Unsurprisingly, the Moran I values calculated on the belonging matrices are higher for SFCM and SGFCM, indicating stronger spatial structures in the classifications.
...
It appears that out of `r length(spdiag_3$SpConsistSamples)` permutations, the observed values of spatial inconsistency of SGFCM is always lower than the one of SFCM. The difference is this significant at the thresold `r 1/length(spdiag_3$SpConsistSamples)`=1/`r length(spdiag_3$SpConsistSamples)`
...
One can obtain many descriptive information about the final groups with three functions of *geocmeans* : 

* summarizeClusters : calculate summary statistics for each group for a given dataset by using the belonging matrix as weights (sticking with the fuzzy spirit).
...
That's all folks ! These are the enhancements for the next version
