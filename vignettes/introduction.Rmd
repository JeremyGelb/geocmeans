...
All the clustering algorithms of this package depends on a initial state that might influence the results. A good pratice is to check the results to ensure that the groups obtained are stable if we replicate the analysis. The function `boost_group_validation` proposes an approach based on boostraping.
...
We can now check if the groups are stable accross simualtions.
...
The Jaccard index is calculated. A greater value means a more stable cluter. Values above 0.75 indicate a stable cluster. Values between 0.5 and 0.75 indicate a cluster wiht some uncertainty. Under 0.5, the cluster tend to dissolve during the iterations. In the presented case, the group 3 seems to be less stable and tends to dissolve. In that context, retaining only three clusters could be a solution.

We can aslo check the values of the centers of the group 3 accross the 1000 iterations.
...
```
It appears that the center values for NO2 and PM2.5 are bimodal, that might explain why this group tends to dissolve.
