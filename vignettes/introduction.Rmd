...
All the clustering algorithms of this package depends on a initial state that might influence the results. A good practice is to check the results to ensure that the groups obtained are stable if we replicate the analysis. The function `boost_group_validation` proposes an approach based on bootstraping.
...
The Jaccard index is calculated. A greater value means a more stable cluster. Values above 0.75 indicate a stable cluster. Values between 0.5 and 0.75 indicate a cluster with some uncertainty. Under 0.5, the cluster tend to dissolve during the iterations. In the presented case, the group 3 seems to be less stable and tends to dissolve. In that context, retaining only three clusters could be a solution.

We can also check the values of the centers of the group 3 across the 1000 iterations.
...
```
It appears that the center values for NO<sub>2</sub> and PM<sub>2.5</sub> are bimodal, that might explain why this group tends to dissolve.
