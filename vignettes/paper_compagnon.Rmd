...
Some people doest no like spiderplots, one can instead use barcharts.
```{r fig.height=15, fig.width=5}
Means <- lapply(spCmeansSummary, function(i){as.numeric(i[8,])})
Means <- data.frame(do.call(rbind, Means))
names(Means) <- colnames(spCmeansSummary$Cluster_1)
Means$Cluster <- rownames(Means)
Means <- reshape2::melt(Means, id.vars = "Cluster")


colors <- c("Cluster_1" = "#386cb0",
            "Cluster_2" = "#6eb97e",
            "Cluster_3" = "#ffff99",
            "Cluster_4" = "#fdc086")
